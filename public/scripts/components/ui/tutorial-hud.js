...
AFRAME.registerComponent('tutorial-hud', {
  ...
  _buildHUD: function () {
    ...
    container.setAttribute('position', `${HUD_CONFIG.pos.x} ${HUD_CONFIG.pos.y} ${HUD_CONFIG.pos.z}`);

    // Main background
    container.appendChild(this._plane(HUD_CONFIG.size.w, HUD_CONFIG.size.h, HUD_CONFIG.colors.bg, { o: 0.80 }));

    // Borders for depth effect
    container.appendChild(this._plane(1.04, 0.26, HUD_CONFIG.colors.borderOuter, { o: 0.35, z: -0.01 }));
    container.appendChild(this._plane(1.02, 0.24, HUD_CONFIG.colors.borderMid, { o: 0.15, z: -0.015 }));
    container.appendChild(this._plane(1.00, 0.22, HUD_CONFIG.colors.borderInner, { o: 0.08, z: -0.02 }));

    // Top accent line
    container.appendChild(this._plane(1.00, 0.015, HUD_CONFIG.colors.borderOuter, { o: 0.5, z: 0.01, y: 0.095 }));

    // Message text element
    const textEl = document.createElement('a-entity');
    textEl.id = 'tutorial-message';
    // Width 0.85 ensures text stays within 85% of HUD width
    textEl.setAttribute('text', 'value: ; align: center; width: 0.85; fontSize: 20; color: #FFFFFF; wrapCount: 30; anchor: center');
  ...
  _processQueue: function () {
    ...
    this._displayMessage(this.currentMessage.text, this.currentMessage.duration);
  },
