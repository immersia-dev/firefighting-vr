...
const testInteractivePanels = () => {
  ...
  assert.equal(testPanels.intro.buttons.length, 1, 'Panel has correct button count');
...
const testTutorialHUD = () => {
  ...
  assert.equal(messageQueue[0].text, 'Hello', 'Message text is stored correctly');
...
const testTrainingManager = () => {
  ...
  let currentIndex = 0;
  currentIndex++;
  assert.equal(states[currentIndex], 'movement_select', 'State progression works');
  ...
  assert.equal(trainingSteps[trainingSteps.length - 1], 'done', 'Final step is done');

  // Test 4: Callback simulation
  let callbackFired = false;
  const callbacks = {
    nextPhase: () => { callbackFired = true; }
  };
  callbacks.nextPhase();
