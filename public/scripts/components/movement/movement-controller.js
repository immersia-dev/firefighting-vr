...
AFRAME.registerComponent('movement-controller', {
  ...
  init: function () {
    ...
    if (window.DEBUG_CONFIG && window.DEBUG_CONFIG.LOG_CONTROLS) {
      window.debugLog('MovementController', 'Initialized with mode:', this.mode);
    }
  ...
  setMode: function (newMode) {
    ...
    if (leftController && leftController.components['teleport-controls']) {
      leftController.components['teleport-controls'].data.enabled = newMode === 'teleport' && this.data.enabled;
    }
  ...
  setEnabled: function (enabled) {
    ...
    const leftController = document.querySelector('#left-hand-controller');
    if (leftController && leftController.components['teleport-controls']) {
      leftController.components['teleport-controls'].data.enabled = enabled && this.mode === 'teleport';
    }
  },
